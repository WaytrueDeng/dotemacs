...
\(fn ENGLISH-SOURCE OTHER-SOURCE &optional ISM-TYPE)" nil nil)
...
If called interactively, enable Sis-Context mode if ARG is
positive, and disable it if ARG is zero or negative.  If called
from Lisp, also enable the mode if ARG is omitted or nil, and
toggle it if ARG is `toggle'; disable the mode otherwise.
...
With prefix ARG, enable Sis-Global-Context mode if ARG is positive;
otherwise, disable it.  If called from Lisp, enable the mode if
ARG is omitted or nil.

Sis-Context mode is enabled in all buffers where
`sis-context-mode' would do it.
